            ...
            case "temprank":
                ...
                if ($sender->hasPermission("temprank.command")) {
                    if (isset($args[0])) {
                        if ($args[0] === "set") {
                            if (count($args) < 4) {
                                $sender->sendMessage("§cuasage: /temprank set <player> <rank> <time>\n§cFor Example: /temprank set Steve Admin 1d12h\nYear = y, Month = m, Day = d, Hour = h, Minute = i, Seconds = s");
                                return true;
                            }
                            ...
                            if (count($args) < 2) {
                                $sender->sendMessage(C::RED . "usage: /temprank remove <player>");
                                return true;
                            }
                            $playername = $args[1];
                            $this->removeRank($playername);
